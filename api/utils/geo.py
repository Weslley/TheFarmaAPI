...
def get_mais_visualizados(latitude,longitude,radius,data_inicio,data_final):
    sql = """
	...
	INNER JOIN api_produto ON api_produto.id = api_apresentacao.produto_id
	INNER JOIN api_fabricante ON api_fabricante.id = api_produto.laboratorio_id) AS distances
    WHERE distance < {radius} AND data_criacao BETWEEN '{data_inicio}' and '{data_final}'
    ...
    OFFSET 0;
    """.format(latitude=latitude,longitude=longitude,radius=radius,data_inicio=data_inicio.strftime('%Y-%m-%d'),data_final=data_final.strftime('%Y-%m-%d'))
    #instancia o cursor
    cursor = connection.cursor()
    #tenta recuperar
    try:
